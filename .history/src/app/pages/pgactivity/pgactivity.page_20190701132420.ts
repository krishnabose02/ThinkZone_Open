...
export class PgactivityPage {
  ...
  async getactivitydetails(month, week, subject){
    this.selected_month = month;
    this.selected_week = week;
    this.selected_subject = subject;

    this.activity_heading = 'Month('+this.selected_month+') - Week('+this.selected_week+') - '+this.selected_subject;
  ...
  async setActivityStatus(all_activities){
    const loading = await this.loadingController.create({});
    await loading.present();
    await this.api.gettchactivitybyuser(this._userid, this.program, this.subject, this.selected_month, this.selected_week).subscribe(res => {
