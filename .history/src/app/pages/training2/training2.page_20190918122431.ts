  ...
  async complete_activity(){
    if(this.isTrainingSession_alreadySaved){
      ...
      if(!this.isVisited_worksheet){
        this.showAlert('Worksheet not viewed', '', 'No worksheet is viewed !!!');
      }else if(!this.isVisited_video){
        this.showAlert('Videos not viewed', '', 'No videos are viewed !!!');
      }else if(!this.user_selected_quiz_arr.length != this.quiz.length){
        this.showAlert('Quiz not complete', '', 'Please answer all the quiz questions !!!');
      }else{
        this.save();
      }
    }
