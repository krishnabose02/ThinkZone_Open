...
export class Ecactivity2Page {
  ...
  constructor(
    ...
    .then( (data) => {
      ...
      this.showAlert('Directory exists','','Directory exists: '+this.jcError);
      for( let ii = 0; ii < data.length; ii += 1){
        ...
        if( thisElem.type.toLowerCase() === "application" && thisElem.canWrite){
          ...
          this.basePathSD = thisElem.filePath;
          break;
        }
      }
      if( !this.basePathSD){
      ...
      let tag = "getExternalSdCardDetails";
      this.jcError += "\n\n" + tag + ":ERR:" + JSON.stringify( errData);
    });


    let path = this.file.externalRootDirectory;
    let new_path = 'file:///storage/extSdCard/'; //path.substring(path.indexOf('s'))
    this.file.checkDir(new_path, 'mydir').then(_ => {
      console.log('Directory exists');
      this.showAlert('Directory exists','','Directory exists: '+new_path);
    }).catch(err =>{
      console.log('Directory doesnt exists');
      this.showAlert('Directory doesnt exists','','Directory doesnt exists: '+new_path);
    });
