...
export class Ecactivity2Page {
  ...
  constructor(
    ...
    this.diagnostic.requestExternalStorageAuthorization().then(val => {
      this.showAlert('xxxxxxxx','','xxxxxxxx val: '+JSON.stringify(val));
      if(val){
        this.diagnostic.getExternalSdCardDetails().then(details => {
          this.showAlert('xxxxxxxx','','xxxxxxxxdetails[0]: '+JSON.stringify(details.length));
          details.forEach(function(detail){
            this.showAlert('xxxxxxxx','','xxxxLoop detail: '+JSON.stringify(detail));
          });
        });
      }
    });
