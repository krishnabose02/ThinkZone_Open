    ...
    this.diagnostic.requestExternalStorageAuthorization().then(val => {
      if(val){
        this.diagnostic.getExternalSdCardDetails().then(details => {
          ...
          this.showAlert('Full Filepath','',''+this.filepath_full);
          let voption: VideoOptions = {
            volume: 0.5,
            scalingMode: 0.5
          };
          // video play working
          /*this.videoPlayer.play(this.filepath_full,voption).then(() => {
            console.log('video completed');
            }).catch(err => {
            console.log(err);
            });
          */
          this.doc_filepath_full = this.sdcard_path+'/Thinkzone/DOCS/test.pdf';
          this.showAlert('file.externalRootDirectory','',''+this.file.externalRootDirectory);
          this.file.readAsBinaryString(this.file.externalRootDirectory+'/Thinkzone/DOCS/test.pdf', 'application/pdf').then(() => {
            console.log('File is opened');
            this.showAlert('File is opened','','');
          })
          .catch(e => {
            console.log('Error opening file', e);
            this.showAlert('Error opening file','',''+e);
          });
        });
      }
    });
