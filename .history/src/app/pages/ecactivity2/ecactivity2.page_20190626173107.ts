...
export class Ecactivity2Page {
  ...
  async open_document(){
    let filename = 'test', file_ext = 'jpg', filename_new = Date.now(), file_type = 'image/jpeg';
    ...
    this.doc_filepath_full = this.sdcard_filepath+'/Thinkzone/DOCS';
    this.showAlert('file.externalApplicationStorageDirectory','',''+this.file.externalApplicationStorageDirectory);

    // copy file and show
    this.file.copyFile(this.doc_filepath_full, filename+'.'+file_ext, this.file.externalApplicationStorageDirectory, filename_new+'.'+file_ext).then(result => {
