...
export class MakepaymentPage {
    ...
    const loading = await this.loadingController.create({});
      ...
      await this.api.getalltchpaymentdetailsbystudentid(sid)
        .subscribe(res => {
          ...
          if(res.length > 0){
            res.forEach(element => {
              this.total_paid_amount += parseInt(res.amount);
            });
            console.log('###this.total_paid_amount: '+this.total_paid_amount);
  ...
  calculate_pending(){
    ...
    this.total_pending_amount = this.total_payable_amount - parseInt(this.pay_amount);
  ...
  async makepayment(){
    ...
    if(this.payment_type == undefined || this.payment_type == null || this.payment_type == '') {
